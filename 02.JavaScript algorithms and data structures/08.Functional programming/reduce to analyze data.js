...
function getRating(watchList) {
	...
	// let temp = 0; //to keep track of num of matches
	// var test = watchList.reduce((acc, curr) => {
	// 	if (curr.Director === 'Christopher Nolan') {
	// 		acc += parseFloat(curr.imdbRating);
	// 		temp++;
	// 	}
	// 	return acc;
	// }, 0);
	// let averageRating = test / temp;

	let ratings = watchList.filter(value => value.Director === 'Christopher Nolan').map(ele => parseFloat(ele.imdbRating));
	let averageRating = ratings.reduce((acc, curr) => acc + curr) / ratings.length;
...
console.log(getRatingF(watchList));